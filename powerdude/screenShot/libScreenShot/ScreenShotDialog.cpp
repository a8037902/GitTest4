...
ScreenShotDialog::ScreenShotDialog(QWidget *parent)
	: QDialog(parent), m_curHwnd(0), m_resizeType(None), m_screenShotStep(SelectArea)
	, m_pCtrlPanel(0), m_pDrawerPanel(0), m_pDrawable(0), m_bIsRenderPaint(false), m_pCurProperty(0), m_bKeyControlPressed(false)
{
	ui.setupUi(this);

	setMouseTracking(true);
	setWindowFlags(Qt::FramelessWindowHint | Qt::Tool | Qt::CustomizeWindowHint | Qt::WindowStaysOnTopHint);
...
BOOL CALLBACK ScreenShotDialog::EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	ScreenShotDialog* pThis = reinterpret_cast<ScreenShotDialog*>(lParam);
...
void ScreenShotDialog::initScreenShot()
{
	::EnumWindows(&ScreenShotDialog::EnumWindowsProc, reinterpret_cast<LPARAM>(this));
...
bool ScreenShotDialog::getWindowRect(int x, int y, LPRECT lpRect)
{
	bool bResult = false;
...
void ScreenShotDialog::paintEvent(QPaintEvent *pPaintEvent)
{
	...
	QDialog::paintEvent(pPaintEvent);
}

void ScreenShotDialog::updateRects(QPoint& startPoint, QPoint& endPoint)
{
...
void ScreenShotDialog::updateDisplayInfo(QPoint& point)
{
	if (!ui.displayInfo->isHidden()) {
		QString strInfo;
...
void ScreenShotDialog::leftButtonPressedMouseMoveEvent(QMouseEvent *pMouseEvent)
{
	if (screenShotStep::SelectArea == m_screenShotStep) {
...
void ScreenShotDialog::leftButtonUnPressedMouseMoveEvent(QMouseEvent *pMouseEvent)
{
	if (screenShotStep::SelectArea == m_screenShotStep) {
		setCursor(Qt::ArrowCursor);
	}
...
void ScreenShotDialog::mouseDoubleClickEvent(QMouseEvent *pMouseEvent)
{
...
void ScreenShotDialog::mouseMoveEvent(QMouseEvent *pMouseEvent)
{
...
void ScreenShotDialog::mousePressEvent(QMouseEvent *pMouseEvent)
{
...
void ScreenShotDialog::mouseReleaseEvent(QMouseEvent *pMouseEvent)
{
	if (pMouseEvent->button() & Qt::LeftButton) {
		ui.displayInfo->hide();
		if (screenShotStep::SelectArea == m_screenShotStep) {
...
void ScreenShotDialog::showCtrlPanel()
{
...
void ScreenShotDialog::keyPressEvent(QKeyEvent *pKeyEvent)
{
...
void ScreenShotDialog::keyReleaseEvent(QKeyEvent *pKeyEvent)
{
	int key = pKeyEvent->key();
	switch (key) {
	case Qt::Key_Control:
...
void ScreenShotDialog::onCancelClicked()
{
	this->accept();
}

void ScreenShotDialog::onFinishClicked()
{
	ui.textEdit->hide();
...
void ScreenShotDialog::onUndoClicked()
{
	if (!m_objects.empty()) {
		IDrawable* pDrawable = m_objects.back();
		m_objects.pop_back();
...
void ScreenShotDialog::onRadioButtonRectangleToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonElipseToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonBrushToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonArrowToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonMaskToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonTextToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonTransparentToggled(bool isToggled)
{
	if (isToggled) {
...
void ScreenShotDialog::onRadioButtonPenSizeToggled(bool isToggled)
{
	if (isToggled) {
		PenSizeEnum size;
...
void ScreenShotDialog::onRadioButtonMaskSizeToggled(bool isToggled)
{
	if (isToggled) {
		MaskSize size;
...
void ScreenShotDialog::onTextAreaChanged()
{
...
void ScreenShotDialog::onColorSelected()
{
...
void ScreenShotDialog::onCurrentIndexChanged(int index)
{
...
QToolButton* ScreenShotDialog::getToolButton(int color)
{
