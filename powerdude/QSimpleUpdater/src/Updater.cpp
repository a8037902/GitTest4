...
bool check_updatefile(const QString& url) {
	...
	socket.connectToHost(baseUrl.host(), 80);
...
void Updater::onReply (QNetworkReply* reply)
{
	...
	if (!check_updatefile(m_downloadUrl)) {
		setUpdateAvailable(false);
		emit checkingFinished(url());
		return;
	}
...
void Updater::setUpdateAvailable (const bool available)
{
    ...
    if (updateAvailable() && (notifyOnUpdate() || notifyOnFinish()))
    {
        ...
        box.setInformativeText (tr ("当前无可用更新"));
        box.setText ("<h3>"
                     + tr ("当前是最新版 %1").arg (moduleName())
                     + "</h3>");
	...
	if (updateAvailable())
	{
		...
		box.setStandardButtons(QMessageBox::Close);
		//box.setInformativeText(QString::fromLocal8Bit("当前无可用更新"));
		box.setText("<h3>"+ QString::fromLocal8Bit("当前是最新版%1").arg(moduleName())+ "</h3>");
		box.setText("<h3>" + QString::fromLocal8Bit("%1是最新版").arg(moduleName()) + "</h3>");
		box.exec();
	}
#endif
}
